...
x expr = \thisrowno{0},
y expr = \thisrowno{1},
] {piecewise_fit_target_scell_usage_s1e3.csv};
...
%    },
    x filter/.code={
        \pgfmathparse{\thisrow{cellset_length}>2000 ? \thisrow{rsrp_50_gap_target_other_pcells} : nan}
        \pgfmathresult
    },
    y filter/.code={
        \pgfmathparse{\thisrow{cellset_length}>2000 ? \thisrow{target_scell_usage} : nan}
        \pgfmathresult
    },
]
table[
    x=rsrp_50_gap_target_other_pcells,
    y=target_scell_usage,
    col sep=comma,
]
{processed_grid_rss_cell_s1e3_t_5e-05_all.csv};

\node[center, text=myred, rotate=0] at (axis cs: 20,0.25) {\bf Spearman};
\node[center, text=myred, rotate=0] at (axis cs: 20,0.1) {\bf Corr: 0.66};

%\legend{Fit model, Data points};
